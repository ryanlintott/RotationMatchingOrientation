...
public struct RotationMatchingOrientationViewModifier: ViewModifier {
    ...
    func changeContentOrientation(interfaceOrientation: InterfaceOrientation?, allowedOrientations: [InterfaceOrientation]) {
        if let interfaceOrientation = interfaceOrientation, allowedOrientations.contains(interfaceOrientation) {
            contentOrientation = interfaceOrientation
        }
        
        if contentOrientation == nil {
            contentOrientation = allowedOrientations.first(where: { InterfaceOrientation.allCases.contains($0)}) ?? interfaceOrientation ?? .portrait
        }
    }
    ...
    func changeOrientations() {
        if isOn {
            withAnimation(animation) {
                ...
                changeContentOrientation(interfaceOrientation: newOrientation, allowedOrientations: allowedOrientations)
                print("Device: \(String(describing: newOrientation)) Content: \(String(describing: contentOrientation))")
            }
        }
    }
    ...
    public func body(content: Content) -> some View {
        ...
        .onChange(of: allowedOrientations) { newValue in
            ...
            changeContentOrientation(interfaceOrientation: interfaceOrientation, allowedOrientations: newValue)
        }
    }
}
